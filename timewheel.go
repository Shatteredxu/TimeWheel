...
func (tw *TimeWheel) AddTimer(delay time.Duration, key interface{}, job Job,par interface{}) {
	if delay < 0 {
		return
	}
	tw.addTaskChannel <- Task{delay: delay, key: key, job: job,parameter: par}
}
